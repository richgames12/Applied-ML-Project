...
class TrainAndEval():
    ...
    def _k_fold(self, features, k:int = 10):
        #K fold is really supposed to be used for hyperparameter tuning, it's not really doing anything here
        ...
        if(isinstance(self.model, MultiheadEmotionCNN)):
            ...
            for fold, (train_indcs, test_indcs) in enumerate(fol_obj.split(train_features)):
                train_dataloader = DataLoader(train_dataset, batch_size=2, sampler=torch.utils.data.SubsetRandomSampler(train_indcs))
                test_dataloader = DataLoader(train_dataset, batch_size=2, sampler=torch.utils.data.SubsetRandomSampler(test_indcs))
    ...
    def _write_confusion_matrix(self, emote_ground_truth: np.ndarray, predictions: np.ndarray):
        included_labels = {key: value for key, value in EMOTION_LABELS.items() if key in (emote_ground_truth + predictions)}        
        evaluator = ModelEvaluator(included_labels)
